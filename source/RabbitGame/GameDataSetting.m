...
@interface GameDataSetting(private)


@end

@implementation GameDataSetting
...
{
    gamelinksinfor.position = ccp(20,394);
}
...
-(void)analyseData:(GameScene *)delegate levelNum:(int)level{
    ...
    if (level==1 || level==2 || level==3  || level==4 || level==5 || level==6) {
        //起始点的坐标，左下角
        leftx = 9.6f + 25;   //25是半径，从而得到第一个的中心点
        lefty = 50.7 + 25;
    ...
    [self updateGamelinksinforPosition];
    [gameScene addChild:gamelinksinfor];
    
    //如果不是教程，读取存档
    if ([Globel shareGlobel].isInCourse) {
        [self setLinkToGrids:grids X:0 Y:5 type:Link_Type_House level:1];
        ...
        while (YES) {
            ...
            if (grid.link==nil) {
                ...
                if (link.type == Link_Type_Rabbit) {//是兔子的话，则上一层
                    [gameScene addChild:link z:(7 - grid.y + 70)];
                    //[gameScene addChild:link z:1];
                }else {
                    [gameScene addChild:link z:(7-grid.y)];
                }
                [gameScene addPeopleOrRevolute:link];
...
//-------------------------------------------------非彩虹球的情况---------------------------------------------
        ...
        NSMutableArray *allfind = [NSMutableArray array];//二维数组  ，存储找到的Link
...
NSInteger sortObjectsByTag(id obj1, id obj2, void *context)
{
...
-(NSMutableArray *)getAllRabbitLevel1Array{
    ...
    NSArray *newarr = [rabbitArray sortedArrayUsingFunction:sortObjectsByTag context:nil];
...
-(void)moveAllRabiit{
    ...
    for (int i = 0; i<len1; i++) {
        ...
        //移动兔子
        [link moveRabbitToTarget: ccp(targetgrid.position.x, targetgrid.position.y + RabbitOffsetY)];
        //更换显示层次
        [gameScene reorderChild:link z:(7 - link.pgrid.y + 70)];
...
-(void)showGetScoreEffect:(Link *)link isdouble:(BOOL)twoDouble position:(CGPoint)pos{
    ...
    CCLabelAtlas *labelscore = [CCLabelAtlas labelWithString:scorestr charMapFile:@"text1.png" itemWidth:16 itemHeight:25 startCharMap:'0'];
    labelscore.anchorPoint = ccp(0.5,0.5);
    labelscore.scale = 0.7f;
    //容器
...
-(void) showLostScoreEffect:(Grid *)grid score:(int)num{
    ...
    //加入容器
    [gameScene addChild:labelscore z:101];
...
-(void)showBomEffect:(Grid *)grid{
    CCAnimation *bom = [CCAnimation animationWithFrame:@"zd_" frameCount:20 delay:0.1];
    CCSprite *sp = [CCSprite spriteWithSpriteFrameName:@"zd_01.png"];
    
    
    //[grid addChild:sp];
    [gameScene addChild:sp z:99];
    sp.position = ccp(grid.position.x,grid.position.y + 25);
...
-(void)recoverRooms{
    NSMutableArray *gamerooms = [gameScene getLevelConfigDate:@"gamerooms"];
    
    int index = 0;
    CCArray *allchilds = gameScene.children;
    ...
    for (int i=0; i<len; i++) {
        ...
        if ([ob isKindOfClass:([GameRoom class])]) {
            ...
            if ([gamerooms count] <= index) {
                return;
            }
            
            NSMutableDictionary *diction = [gamerooms objectAtIndex:index];
            
            Link_Type type  = [[diction objectForKey:@"type"] intValue];
            int level = [[diction objectForKey:@"level"] intValue];
...
-(void)recoverHistoryGridsData:(NSMutableArray *)grids :(NSMutableArray *)historygrids{ 
    ...
    //这个时候仓库还没有创建，所以需要在0.5后恢复仓库,
    [self performSelector:@selector(recoverRooms) withObject:nil afterDelay:0.5];
        ...
        for (int a=0; a<count; a++) {
            ...
            if (keys==nil || [keys count]==0) {//没有link
                ...
                if (grid.link.type != type || grid.link.level != level) {
                    ...
                    if (new.type == Link_Type_Rabbit) {//是兔子的话，则上一层
                        [gameScene addChild:new z:(7 - grid.y + 70)];
                    }else {
                        [gameScene addChild:new z:(7-grid.y)];
                    }
                    new.pgrid = grid;
...
-(void)releaseGridsTouchRegist{
    ...
    for (int i=(len-1); i>=0; i--) {
        ...
        for (int a=0; a<count; a++) {
            Grid *grid = [lines objectAtIndex:a];
            [[CCTouchDispatcher sharedDispatcher]removeDelegate:grid];
        }
    }
}
