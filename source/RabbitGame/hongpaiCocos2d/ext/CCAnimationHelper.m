...
+(CCAnimation*) animationWithFrame:(NSString*)frame frameCount:(int)frameCount delay:(float)delay
{
	...
	for (int i = 0; i < frameCount; i++)
	{
		//NSString* file = [NSString stringWithFormat:@"%@%03i.png", frame, i+1];
        NSString* file = [NSString stringWithFormat:@"%@%02d.png", frame, i+1];
...
+(CCAnimation*) animationWithNames:(NSArray *)names delay:(float)delay
{
	// load the ship's animation frames as textures and create a sprite frame
	...
	{
		CCSpriteFrameCache* frameCache = [CCSpriteFrameCache sharedSpriteFrameCache];
		CCSpriteFrame* frame = [frameCache spriteFrameByName:[names objectAtIndex:i]];
		[frames addObject:frame];
	}
