...
// 分数和成就的对应，有个极限性，突破极限值获得成就
-(void)addScoreLimitPair:(int)limit forKey:(NSString*)key {
    [scoreLimitPairs setObject:[NSNumber numberWithInt:limit] forKey:key];
}

+ (int)getScore:(NSString*)identifier
{
    int score = [[CCLocalDataLoader sharedLoader] intForKey:identifier];
    return score;
}

+ (bool)stepUpScore:(NSString*)identifier
{
    int scoreOld = [GameKitHelper getScore:identifier];
    [[CCLocalDataLoader sharedLoader] setIntValue:scoreOld + 1 forKey:identifier];
    return true;
}

+ (bool)setScore:(int)score category:(NSString*)identifier
{
    int scoreOld = [GameKitHelper getScore:identifier];
    if (score > scoreOld) {
        [[CCLocalDataLoader sharedLoader] setIntValue:score forKey:identifier];
        return true;
    }
    return false;
}

-(void)stepUpScoreForKey:(NSString*)key
{
    if ([GameKitHelper stepUpScore:key]) {
        NSNumber *number = [scoreLimitPairs objectForKey:key];
        int limit = [number intValue];
        int score = [GameKitHelper getScore:key];
        float percent = (float)score / (float)limit * 100.0f;
        [self reportAchievementWithID:key percentComplete:percent];
    }
}

-(void)submitScore:(int)score forKey:(NSString*)key
{
    if ([GameKitHelper setScore:score category:key]) {
        NSNumber *number = [scoreLimitPairs objectForKey:key];
        int limit = [number intValue];
        float percent = (float)score / (float)limit * 100.0f;
        [self reportAchievementWithID:key percentComplete:percent];
...
-(id) init
{
	if ((self = [super init]))
	{
        ...
        scoreLimitPairs = [[NSMutableDictionary alloc] initWithCapacity:20];
        [self addScoreLimitPairs];
	}
					   ...
					   timeScope:(GKLeaderboardTimeScope)timeScope 
{
	...
	if (leaderboard != nil)
	{
		...
		{
			[self setLastError:error];
			[delegate onScoresReceived:leaderboard scores:scores error:error];
		}];
	}
}
